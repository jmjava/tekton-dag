#!/usr/bin/env bash
# generate-all.sh — TTS → optional Manim → VHS → compose (portable template).
#
# Usage:
#   cd <your-demos-dir>   # directory containing this script, compose.sh, narration/, etc.
#   ./generate-all.sh
...
# Optional Manim: space-separated scene class names from animations/scenes.py (empty = skip)
...
[[ "$SKIP_VHS" == "true" ]] || [[ -x "$VHS_BIN" ]] || { echo "ERROR: vhs not found ($VHS_BIN)" >&2; exit 1; }
...
echo "╔══════════════════════════════════════════════╗"
echo "║  Demo video generator (portable)             ║"
echo "╚══════════════════════════════════════════════╝"
...
if [[ "$DRY_RUN" == "true" ]]; then
    echo "Planned: TTS, Manim (if MANIM_SCENES set), VHS, compose.sh"
    exit 0
fi
