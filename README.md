...
| [M15](milestones/milestone-15.md) | **Implemented** | Control-plane hygiene: idempotent StackRun→PipelineRun, GHA `--skip-operator`, Triggers `prNumber`, Flask/GUI promote approval, soak `ready==total`, dead PipelineRun builders. |
| [M16](milestones/milestone-16.md) | **Not started** | Team CR as Flask/GUI source of truth, Stack admission webhook + certs, retire PipelineRun escape hatches, `stack-pr-continue` field, demo narration/MP4 rebuild. |
