...
Once all services are running, let's trace a request through the stack. A user hits the Vue front-end. The front-end makes an API call to the BFF. In the stack definition, the front-end's propagation role is originator — it sets the dev-session header on all outgoing requests. That header is named x-dev-session in configuration.
