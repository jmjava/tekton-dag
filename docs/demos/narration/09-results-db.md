...
Let's verify. The verify-results-in-db script queries the Tekton Results API and shows stored records. Here you can see our bootstrap run and two PR runs with their start times, durations, and outcomes.
...
The GET /api/runs endpoint on the orchestrator also reads from the Kubernetes API to show recent runs, giving you both live status and historical data.
