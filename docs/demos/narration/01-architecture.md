...
The Kubernetes operator is a key component of this architecture. It allows for more granular control over stack management through Stack and StackRun custom resources. The operator is the default runtime path. Helm operator enabled defaults on, so a chart install gets Stack, StackRun, and Team resources without a feature flag.
