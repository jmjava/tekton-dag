What happens when a test fails in the PR pipeline and you need to debug? You do not SSH into a pod or add print statements. You debug locally — with your IDE, your breakpoints, your tools — while connected to the live cluster.
...
The pipeline even supports this during PR testing. Because the intercept setup is already wired, you can attach mirrord to the PR pod to debug a failing test with the exact request that caused the failure.
